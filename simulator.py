...
def main(def_args=sys.argv[1:]):
    ...
    end_date = curr_date.replace(hour=20, minute=0) - timedelta(days=120)  # Stop at 120 days ago
    
    for day in (start_date + timedelta(n) for n in range(days_before)):
        if day < end_date:
...
def arguments(argsval):
    ...
    parser.add_argument('-ue', '--user_email', type=str, required=False,
                        ...
                        If not specified, the global config is used.""")
    parser.add_argument('-db', '--days_before', type=int, default=730,
